<template>
  <div class="card mb-4 headerCard" style="border-radius: 20px; transform: rotate(-1deg)">
    <div class="card-header container" style="border-top-left-radius: 20px; border-top-right-radius: 20px"
      ...
      <div class="row mt-1">
        <!--
          Character name
        -->
        <h1 class="col hiden-element mb-0" :style="{ 'color': mainTitleColor }" style="font-size: 26px;">{{ nameInputValue
        }}</h1>
        <div class="input-group input-group-lg w-50 ml-2 to-exclude col">
          <input type="text" class="form-control" placeholder="Name" aria-label="Name" v-model=nameInputValue
        ...
        <div class="nav col" style="justify-content: flex-end; align-items: center; align-content: center; gap: 10px;">
          <a data-toggle="tab" href="#nav-about" role="tab" aria-controls="nav-about"
            :style="{ 'color': mainAboutColor, 'background': mainAboutBackgroundColor }" style="  
                        cursor: pointer;
                        font-weight: 400;
                        text-align: center;
            ...
            <SuperColorPicker icon="chess-board" button-color="dark" base-color="#ffffff"
              @color-changed="changeAboutBackgroundColor" class="to-exclude" />
            <SuperColorPicker icon="font" button-color="light" base-color="#000000" @color-changed="changeAboutColor"
              class="to-exclude" />
          </a>
          <a data-toggle="tab" href="#nav-story" role="tab" aria-controls="nav-secondary" class=""
            :style="{ 'color': mainStoryColor, 'background': mainStoryBackgroundColor }" style="
                        cursor: pointer;
                        font-weight: 400;
                        text-align: center;
                        vertical-align: middle;
            ...
            Story
            <SuperColorPicker icon="chess-board" button-color="dark" base-color="#ffffff"
              @color-changed="changeStoryBackgroundColor" class="to-exclude" />
            <SuperColorPicker icon="font" button-color="light" base-color="#000000" @color-changed="changeStoryColor"
              class="to-exclude" />
          </a>
        </div>
      </div>
    ...
    -->
    <div class="card-body" :style="{ 'background': mainBackgroundColor }"
      style="border-bottom-right-radius: 20px; border-bottom-left-radius: 20px;">
      <div class="container">
        <SuperColorPicker icon="chess-board" button-color="dark" base-color="#ffffff"
          @color-changed="changeBackgroundColor" class="to-exclude" />
        <div class="row py-4">
          <div class="traits col-12 pl-3 col-lg-3 d-flex justify-content-start justify-content-lg-center">
            <div>
              <div>
                <!--
                          tags
                        -->
                <div v-for="(tag, index) in tags" style="
                  ...
                  {{ tag }}
                  <button @click="removeTag(index)" type="button" class="close margin-top to-exclude" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                  </button>
                </div>
                <SuperColorPicker icon="chess-board" button-color="dark" base-color="#ffffff"
                  @color-changed="changeTagBgColor" class="to-exclude" />
                <SuperColorPicker icon="font" button-color="light" base-color="#000000"
                  @color-changed="changeTagTextColor" class="to-exclude" />
                <form @submit.prevent="addTag" class="to-exclude">
                  <div class="input-group my-2">
                    <input v-model="inputValue" type="text" class="form-control" placeholder="tag" aria-label="tag"
                      aria-describedby="add-tag" />
                    <div class="input-group-append">
                      <button class="btn btn-outline-dark" type="submit" id="add-tag" :disabled="!inputValue">
          ...
          <div class="resume col-12 col-lg-9">
            <p :style="{ 'color': mainDescriptionColor }" style="white-space: pre-line;" class="hidden-element cringe">{{
              descriptionInputValue }}</p>
            <div class="input-group to-exclude">
              <textarea class="form-control" aria-label="description" placeholder="Write your description here!"
                :style="{ 'color': mainDescriptionColor }" v-model="descriptionInputValue"></textarea>
              <SuperColorPicker icon="font" button-color="light" base-color="#000000"
                @color-changed="changeDescriptionColor" class="to-exclude" />
            </div>
            <p></p>
          </div>
        </div>
